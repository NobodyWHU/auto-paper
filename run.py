    ...
    with open(report_filename, 'a', encoding='utf-8') as f_report:
        ...
        for result in new_papers:
            ...
            try:
                ...
                ### 4. 📄 核心架构图还原 (Mermaid语法)
                (由于纯文本无法展示原论文图片，请你务必根据论文中最核心的模型架构图或主流程图，使用 Mermaid 语法（```mermaid ... ```）绘制一个结构清晰、逻辑严密的流程图或架构图。必须准确还原论文图表中的关键模块、数据流向及交互关系。)
                """
                ...
                - **(请务必从论文中提取最具代表性的实验结果对比数据，使用 Markdown 表格的形式展示出来，用真实数据支撑你的结论)**
